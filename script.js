...
function initialize(products) {
    ...
    if (category.value === lastCategory && searchTerm.value.trim() === lastSearch) {
      ...
      if (category.value === 'All') {
        ...
        for (let i = 0; i < products.length; i++) {
          ...
          if (products[i].category === lowerCaseType) {
  ...
  function selectProducts() {
    ...
    if (searchTerm.value.trim() === '') {
      ...
      // For each product in categoryGroup, see if the search term is contained inside the product title
      // (if the indexOf() result doesn't return -1, it means it is) — if it is, then push the product
      // onto the finalGroup array
      for (let i = 0; i < categoryGroup.length; i++) {
        if (categoryGroup[i].title.indexOf(lowerCaseSearchTerm) !== -1) {
  ...
  function updateDisplay() {
    ...
    if (finalGroup.length === 0) {
      ...
      para.textContent = 'Ничего не найдено!';
  ...
  // fetchBlob uses fetch to retrieve the primaryImageUrl for that product, and then sends the
  // resulting primaryImageUrl display URL and product object on to showProduct() to finally
  // display it
  function fetchBlob(product) {
    // construct the URL path to the primaryImageUrl file from the product.primaryImageUrl property
    let url = 'img/' + product.primaryImageUrl;
    // Use fetch to fetch the primaryImageUrl, and convert the resulting response to a blob
  ...
  function showProduct(objectURL, product) {
    ...
    heading.textContent = product.title.replace(
      product.title.charAt(0),
      product.title.charAt(0).toUpperCase()
    );

    // Give the <p> textContent equal to the product "priceRetail" property, with a $ sign in front
    // toFixed(2) is used to fix the priceRetail at 2 decimal places, so for example 1.40 is displayed
    // as 1.40, not 1.4.
    para.textContent = product.priceRetail.toFixed(2) + 'RUB';

    // Set the src of the <img> element to the ObjectURL, and the alt to the product "title" property
    image.src = objectURL;
